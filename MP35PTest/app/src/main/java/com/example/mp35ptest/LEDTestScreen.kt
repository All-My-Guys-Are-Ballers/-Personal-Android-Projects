...
@Composable
fun LEDTestScreen() {
    val ledControl = DeviceServiceManager.getInstance().getLed(
    );
    Surface(
        modifier = Modifier
            .fillMaxSize()
                        ...
                        modifier = Modifier
                            .clickable {
                                when (it) {
                                    ...
                                    LEDConfigurations.RED_LED_OFF -> ledControl.setLed(LedCode.OPER_LED_RED, false)
                                    LEDConfigurations.YELLOW_LED_ON -> {
                                        if (ledControl == null) Log.d(ContentValues.TAG, "Help")
                                        ledControl.setLed(LedCode.OPER_LED_YELLOW, true)
                                    }
                                    LEDConfigurations.YELLOW_LED_OFF -> ledControl.setLed(LedCode.OPER_LED_YELLOW, false)
                                    LEDConfigurations.GREEN_LED_ON -> {
                                        if (ledControl == null) Log.d(ContentValues.TAG, "Help")
                                        ledControl.setLed(LedCode.OPER_LED_GREEN, true)
                                    }
                                    LEDConfigurations.GREEN_LED_OFF -> ledControl.setLed(LedCode.OPER_LED_GREEN, false)
                                    LEDConfigurations.BLUE_LED_ON -> {
                                        if (ledControl == null) Log.d(ContentValues.TAG, "Help")
                                        ledControl.setLed(LedCode.OPER_LED_BLUE, true)
                                    }
                                    LEDConfigurations.BLUE_LED_OFF -> ledControl.setLed(LedCode.OPER_LED_BLUE, false)
